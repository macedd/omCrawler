...
var pjs = (function(){
    var config = {
            ...
            pageSettings: { }, //phantomjs page settings
            ignoreUrls:[], //same aspect as visited=[]
        };
    ...
    var runner = (function() {
        ...
        ScraperSuite.prototype = {
            ...
            run: function() {
                ...
                function runNext() {
                    if (runCounter < s.urls.length) {
                        ...
                        // avoid repeat visits or ignored urls
                        if (!config.allowRepeatUrls && ( url in visited || url in config.ignoreUrls ) ) {
                            runNext();
                        } else {
                            // scrape this url
            ...
            scrape: function(url, scrapers, complete) {
                ...
                //TODO: centralizar funcoes acima e as abaixo (tudo settings)
                            ...
                            if (scrapers && scrapers.length) {
                                ...
                                scrapers.forEach(function(scraper) {
                                    if (isFunction(scraper)) {
                                        // standard scraper
                                        //log.error(opts.query.length);
                                        suite.addItem(page.evaluate(scraper, opts.query));
                                        checkComplete();
                                        ...
                                        if (scraper.async) {
                                            // start the scrape
                                            page.evaluate(scraper.scraper, opts.query);
        ...
        function init() {
            ...
            //todo:make hook preSuite
            //tool para autenticar
            //refactor para manter contexto no suite (moreurls, auth)
            //page.onInitialized, onLoadStarted, onLoadFinished
            
            /*if () {
                var page = SuiteManager.getPage()
                suite.opts.preSuite
            }*/
...
if (!phantom.args.length) {
    ...
    phantom.args.forEach(function(configFile) {
            try {
                //allow inject json code on stdin
                //try to load injected js if present
